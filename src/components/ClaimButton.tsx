...
export default function ClaimButton({ contractAddress, candidates, handleShow }: Props) {
  const { claiming, checkOwnership, claim } = useWallet()
  const { isConnecting } = useAccount()

  const account = useSelector((store: typeof InitState) => store.account)
    ...
    async function checkStatus() {
      ...
      try {
        ...
        const isEligible = candidates.some(
          address => address.toLowerCase() === (account as string).toLowerCase()
        )
  ...
  return (
    <>
      ...
      {!loading && (
        <>
          {!!account && status === 'not-eligible' && (
            <button disabled className="btn btn-secondary text-lg px-12 py-4 cursor-not-allowed">
              您不是本季节点候选人
            </button>
          )}
          ...
          {status !== 'not-eligible' && status !== 'claimed' && (
            <button
              onClick={handleClick}
